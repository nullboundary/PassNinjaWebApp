(function(passImages, $, undefined) {
	...
	function checkImage() {
		...
		if (window.File && window.FileReader && window.FileList && window.Blob) {
			...
			{
				passBuilder.alertDisplay("error", 'no image selected');
				return false
			}
			...
			if (fsize > 1048576) {
				...
				passBuilder.alertDisplay("error", 'Image file should be less than 1MB!');
				return false
			}
			...
			passBuilder.alertDisplay("error", 'Please upgrade your browser!');
			return false;
		}
	}
		...
		if (checkImage()) {
			...
			img.onload = function() {
				...
				if (errorMessage != "") {
					...
					passBuilder.alertDisplay("error", errorMessage);
	...
	/* set pass images */
	passImages.set = function() {
		setPassImages();
	};

	passImages.onRectClick = function() {
		onImageRectClick();
	};

	passImages.onUpload = function() {
		onImageUpload();
	};

	return passImages; //return the image object

}(passImages = window.passImages || {}, jQuery));