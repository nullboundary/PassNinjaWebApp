(function (passBarcode, $, undefined) {
  ...
  function onBarcodeSubmit() {
    ...
    if (barType != "No Barcode") {
      passTemplate.keyDoc.barcode.format = barcodeShape[barType].format;
      passTemplate.keyDoc.barcode.message = barData;
      passTemplate.keyDoc.barcode.messageEncoding = barEncode;

    } else {
      delete passTemplate.keyDoc.barcode;
  ...
  function onAltText() {
    ...
    if (altValue.length) {
      passTemplate.keyDoc.barcode.altText = altValue;
    } else {
      delete passTemplate.keyDoc.barcode.altText; //remove it from the keydoc
    }
  ...
  function setAltText(barType) {
    ...
    var altValue = passTemplate.keyDoc.barcode.altText;
