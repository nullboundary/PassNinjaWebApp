(function (pb, $, undefined) {
  ...
  function selectRow() {
    ...
    if (tableId == 'loc-table') {
      ...
      if (data.proximityUUID) {
        d3.select('input#beacon-uuid')
          .property('value', data.proximityUUID);
      }

      if (data.major) {
        d3.select('input#beacon-major')
          .property('value', data.major);
      }

      if (data.minor) {
        d3.select('input#beacon-minor')
          .property('value', data.minor);
      }
  ...
  function onUpdateBeacon() {
    ...
    if (inputId == 'beacon-uuid') {
      ...
      beacons[index].major = inputVal;

    } else if (inputId == 'beacon-minor') {

      beacons[index].minor = inputVal;
